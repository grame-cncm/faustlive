...
string getJson(server_connection_info_struct* con_info){
...
server_connection_info_struct* Server::allocate_connection_struct(MHD_Connection *connection, const char *method){
    
    struct server_connection_info_struct *con_info;
    
    con_info = new server_connection_info_struct();
    con_info->init();
...
// If connection is new, a connection structure is allocated
    if (NULL == *con_cls){
        ...
        server_connection_info_struct* con_struct = server->allocate_connection_struct(connection, method);
    ...
    if (0 != *upload_data_size) {
        ...
        *upload_data_size = 0;
        ...
        if(strcmp(url, "/GetJson") == 0){
            ...
            printf("FL_SERVER_DELETE_FACTORY = %s\n", con_info->fSHAKey.c_str());
            
            llvm_dsp_factory* toDelete = fAvailableFactories[con_info->fSHAKey].second;
...
// The datas are stocked in server_connection_info_struct
int Server::iterate_post(void *coninfo_cls, MHD_ValueKind /*kind*/, const char *key, const char */*filename*/, const char */*content_type*/, const char */*transfer_encoding*/, const char *data, uint64_t /*off*/, size_t size){
    ...
    struct server_connection_info_struct *con_info = (server_connection_info_struct*)coninfo_cls;
...
bool Server::getJsonFromKey(server_connection_info_struct* con_info){
    
    printf("Server::getJsonFromKey\n");
...
// Create DSP Factory 
bool Server::compile_Data(server_connection_info_struct* con_info){
    ...
    if(con_info->fSHAKey != ""){
        ...
        if(con_info->fLLVMFactory){
            printf("CompileData available factories with %s\n", con_info->fSHAKey.c_str());
...
// Create DSP Instance
bool Server::createInstance(server_connection_info_struct* con_info){
